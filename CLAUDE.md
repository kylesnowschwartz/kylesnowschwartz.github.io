...
1. **One grid, shared lines.** A single 13-column grid (`repeat(13, 1fr)`, 13 is Fibonacci so column ratios land on φ). One gutter everywhere: `--col-gap: var(--s3)` (21px). Folds land on Fibonacci splits (each ≈ φ): **line 6** (on-site shelf : content = 5 : 8), **line 11** (what : where = 5 : 3), **line 9** (know : believe = 8 : 5); blocks also share the container edges (lines 1 and 14). The Projects panel runs full-width (1 / 14) as a marquee band between the narrative row and the skills row, echoing the wordmark's horizontal energy. Never nudge with negative margins — place by `grid-column` span only.
