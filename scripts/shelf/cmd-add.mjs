...
const GENRES = ['SFF', 'Thriller', 'Horror', 'Literary', 'Nonfiction'];
...
export async function run(opts, command) {
  ...
  if (!opts.dryRun) {
    ...
    log(color.accent(`  ✓ added "${title}" → src/data/books.json (id ${id})`));
    for (const w of warnings) log(color.dim(`  ⚠ ${w}`));
    log(color.dim('  ↳ refresh recommendations: npm run shelf -- fetch && npm run shelf -- embed && npm run shelf -- build'));
  } else {
